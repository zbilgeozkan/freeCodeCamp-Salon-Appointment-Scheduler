...
MAIN_MENU() {
  ...
  then
    echo "Sorry, we don't have any service available for now."
  else
    ...
    then
      ...
      if [[ -z $SERV_AVAIL ]]
      then
        MAIN_MENU "Sorry, we don't have that service. Would you like to try anything else?"
      else
